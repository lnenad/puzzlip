...
#[wasm_bindgen]
pub async fn puzzle_me(mut category: String, difficulty: u32, image: String) -> JsValue {
    ...
    let grid = generate_grid(&img, difficulty, w, h, cx, cy);
...
pub fn rotate_segment(
    ...
    for (idx, mut segment) in &mut grid_enum {
        ...
        {
            let mut i = image::load_from_memory(&segment.p.ri[..]).unwrap();
            ...
            //log(format!("{:?}", segment).as_str());

            return serde_wasm_bindgen::to_value(&segment).unwrap();
        }
    }
